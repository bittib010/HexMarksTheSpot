...
- Parsed Node trees (serialized to JSON for fast re-loading)
- Bookmarks with comments (persisted per-file by SHA-256 hash)

Cache layout:
    cache/
        <sha256_hex>/
            parsed.json      # Serialized Node tree
...
def save_parsed_cache(file_hash: str, root: Node, filepath: str, parser_name: str) -> bool:
    """Save a parsed Node tree to the cache.
    ...
    try:
        ...
        # Save the Node tree
        parsed_path = os.path.join(cache_path, "parsed.json")
        with open(parsed_path, "w", encoding="utf-8") as f:
            json.dump(node_to_dict(root), f, separators=(",", ":"))
...
def load_parsed_cache(file_hash: str) -> Optional[Node]:
    ...
    """
    try:
        parsed_path = os.path.join(_get_cache_path(file_hash), "parsed.json")
        ...
        logger.info(f"Loaded parsed data from cache ({file_hash[:12]}...)")
        return root
    except Exception as e:
...
def has_parsed_cache(file_hash: str) -> bool:
    ...
    return os.path.exists(os.path.join(_get_cache_path(file_hash), "parsed.json"))
