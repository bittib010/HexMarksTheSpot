...
class Node:
    ...
    fg_color: str        # Optional explicit foreground/text color (e.g., anti-forensics red-on-black). None = auto-calculated contrast.
...
- Click synchronization between hex viewer and treeview (green border highlight on selected hex bytes and treeview item)
...
- Parent container highlighting — when a field is selected, its parent struct/section (from the JSON nesting) is visually indicated: sibling fields in the hex viewer get a solid black border (#000000, same style as the selected field's green border but in black), and the parent container's treeview row gets a colored border. This leverages the existing JSON hierarchy so no manual configuration is needed. Tracked via `_tag_to_parent_tag` and `_parent_tag_to_children` mappings built during `_collect_nodes()` DFS.
...
- Unparsed gap-fill — hex viewer always shows every byte of the file; byte ranges not covered by any parser field are inserted as gap entries (light gray) so the hex viewer is the authoritative file view. Gap content is analyzed: all-zero regions show as "Nulled Data" with a concise summary (potential wiping/padding), mostly-zero regions (≥85% zeroes, ≥16 bytes) show as "Mostly Null Data" highlighting only the non-zero byte range, and normal unparsed regions show as "Unparsed Data" with byte count.
...
| `antiforensic` | bool or string | `true` to statically flag, or a Python expression evaluated at parse time (e.g., `"$DatabaseSizeInPages != 0 and $_file_size != $DatabaseSizeInPages * $PageSize"`). If truthy, field gets black bg + red text (highest color precedence). Uses `$field` refs and built-in `$_file_size`. |
...
Auto-generated by `ColorGenerator` in five categories (precedence order):
1. **Anti-forensic** (black bg, red text) — `antiforensic: true` or truthy expression (highest priority)
2. **Deprecated** (muted gray) — `deprecated: true` or reason string
3. **Forensic** (red/warm) — `forensic_value: true` or category string
4. **Informational** (green/teal) — `informational: true`
5. **Default** (alternating two-tone) — everything else

Default (uncategorized) fields alternate between two fixed colors: `DEFAULT_COLOR_A` (#D6DAE0, light blue-gray) and `DEFAULT_COLOR_B` (#4A5568, dark slate gray). This two-tone pattern creates a low-key visual rhythm that makes categorized fields (red forensic, green info, black anti-forensic) pop out prominently. The toggle resets on each new file via `ColorGenerator.reset()`.

Anti-forensic fields use `Node.fg_color` for an explicit red text color that overrides the auto-calculated contrast color. `color_gradient: true` creates progressive lightness variants for child fields.
